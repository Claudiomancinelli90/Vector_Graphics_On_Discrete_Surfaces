...
inline string convert_nanoseconds(int64_t t, log_unit unit) {
  ...
  if (unit == log_unit::microsecond) {
    ...
    return format("$.$$$ms", mus, n2, n1, n0);
  }
  if (unit == log_unit::millisecond) {
    auto mu0 = next_digit(mus);
    auto mu1 = next_digit(mus);
    auto mu2 = next_digit(mus);
    return format("$.$$$ms", ms, mu2, mu1, mu0);
  }
...
#define info_function() auto _info = log::info_scope("$$", __FUNCTION__, "()");
#define time_function() \
  auto _time = log::time_scope(string(__FUNCTION__) + "()");
#define here() log::info("here: $ - $", __FUNCTION__, __LINE__);
#define show(V) log::info(string(#V) + ": $", V);

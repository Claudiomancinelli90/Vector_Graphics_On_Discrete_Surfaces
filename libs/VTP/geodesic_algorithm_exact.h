...
private:
  ...
  void find_separating_point(list_pointer &list); // find the separating point
                                                  // of the windows and the list
  void propagate_windows_to_two_edges(
      list_pointer
          &list); // propagates windows to two edges accross a triangle face

  // pairwise windows checking (Rule 2)
  void check_with_vertices(list_pointer &list);
  windows_state check_between_two_windows(
      interval_pointer &w1,
      interval_pointer
          &w2); // Check two neighbouring crossing windows on same edge
  void pairwise_windows_checking(
      list_pointer &list); // Check crossing windows on same edge

  // main operation
  void propagate_one_windows_list(list_pointer &list);

  // member variables
  // std::multiset<vertex_pointer, Vertex> m_vertex_queue; // priority queue for
  // vertices
...
inline void GeodesicAlgorithmExact::create_pseudo_source_windows(
    vertex_pointer &pseudo_source, bool inside_traversed_area) {
  ...
  for (unsigned i = 0; i < pseudo_source->adjacent_edges().size(); ++i) {
    ...
    if (distance < vert_it->geodesic_distance()) {
      ...
      edge_pointer next_edge =
          vert_it->incident_face()->next_edge(edge_it, pseudo_source);
      vert_it->incident_point() =
          (next_edge->v0() == pseudo_source) ? 0 : next_edge->length();
  ...
  for (unsigned i = 0; i < pseudo_source->adjacent_faces().size(); ++i) {
    ...
    list_pointer list = (edge_it->adjacent_faces()[0] == face_it)
                            ? list = interval_list_0(edge_it)
                            : list = interval_list_1(edge_it);
    ...
    if ((inside_traversed_area) && ((edge_it->v0()->state() != Vertex::FRONT) ||
                                    (edge_it->v1()->state() != Vertex::FRONT)))
...
inline void GeodesicAlgorithmExact::find_separating_point(list_pointer &list) {
  const double LOCAL_EPSILON =
      1e-20 * list->edge()->length(); // numerical issue
  ...
  while (iter != NULL) {
    ...
    double w_sp = w->pseudo_x() - w->pseudo_y() * ((top_x - w->pseudo_x()) /
                                                   (top_y - w->pseudo_y()));
    double distance = GEODESIC_INF;

    // shortest path from the window
    if ((w_sp - w->start() > LOCAL_EPSILON) &&
        (w_sp - w->stop() < -LOCAL_EPSILON)) {
      distance =
          w->d() + sqrt((top_x - w->pseudo_x()) * (top_x - w->pseudo_x()) +
                        (top_y - w->pseudo_y()) * (top_y - w->pseudo_y()));
      w->shortest_distance() = distance;
    } else if (w_sp - w->start() <= LOCAL_EPSILON) {
      distance =
          w->d() +
          sqrt((top_x - w->start()) * (top_x - w->start()) + top_y * top_y) +
          sqrt((w->start() - w->pseudo_x()) * (w->start() - w->pseudo_x()) +
               w->pseudo_y() * w->pseudo_y());
      w->shortest_distance() = distance;
      w_sp = w->start();
    } else if (w_sp - w->stop() >= -LOCAL_EPSILON) {
      distance =
          w->d() +
          sqrt((top_x - w->stop()) * (top_x - w->stop()) + top_y * top_y) +
          sqrt((w->stop() - w->pseudo_x()) * (w->stop() - w->pseudo_x()) +
               w->pseudo_y() * w->pseudo_y());
      w->shortest_distance() = distance;
      w_sp = w->stop();
    }
    ...
    if (distance < top_t.geodesic_distance()) {
      ...
      top_t.incident_point() = (list->start_vertex() == list->edge()->v0())
                                   ? w_sp
                                   : list->edge()->length() - w_sp;
      wlist_sp = w_sp;
  ...
  if (top_t.geodesic_distance() < Tri.top_vertex->geodesic_distance()) {
    ...
    if ((Tri.top_vertex->state() == Vertex::INSIDE) &&
        (Tri.top_vertex->saddle_or_boundary()))
      create_pseudo_source_windows(Tri.top_vertex,
                                   true); // handle saddle vertex
  }
...
inline void
GeodesicAlgorithmExact::propagate_windows_to_two_edges(list_pointer &list) {
  const double LOCAL_EPSILON = 1e-8 * list->edge()->length(); // numerical issue

  interval_pointer iter = list->begin();
  interval_pointer iter_t;

  enum PropagationDirection { LEFT, RIGHT, BOTH };
  ...
  while (!list->empty() && (iter != NULL)) {
    ...
    if (w->sp() < list->sp() - LOCAL_EPSILON) {
      ...
      CalculateIntersectionPoint(list->pseudo_x(), list->pseudo_y(), list->sp(),
                                 0, w->pseudo_x(), w->pseudo_y(), w->stop(), 0,
                                 Intersect_X, Intersect_Y);
      if ((w->stop() < list->sp()) ||
          ((Intersect_Y <= 0) && (Intersect_Y >= list->pseudo_y()) &&
           (Intersect_Y >= w->pseudo_y()))) {
      ...
      // judge the positions of the two windows
      CalculateIntersectionPoint(list->pseudo_x(), list->pseudo_y(), list->sp(),
                                 0, w->pseudo_x(), w->pseudo_y(), w->start(), 0,
                                 Intersect_X, Intersect_Y);
      if ((w->start() > list->sp()) ||
          ((Intersect_Y <= 0) && (Intersect_Y >= list->pseudo_y()) &&
           (Intersect_Y >= w->pseudo_y()))) {
    ...
    case PropagationDirection::LEFT:
      ValidPropagation = compute_propagated_parameters(
          w->pseudo_x(), w->pseudo_y(), w->start(), w->stop(), Tri.left_alpha,
          Tri.left_edge->length(), w, w->d());
    ...
    case PropagationDirection::RIGHT:
      ValidPropagation = compute_propagated_parameters(
          Tri.bottom_edge->length() - w->pseudo_x(), w->pseudo_y(),
          Tri.bottom_edge->length() - w->stop(),
          Tri.bottom_edge->length() - w->start(), Tri.right_alpha,
          Tri.right_edge->length(), w, w->d());
    ...
    case PropagationDirection::BOTH:
      ...
      ValidPropagation = compute_propagated_parameters(
          w->pseudo_x(), w->pseudo_y(), w->start(), w->stop(),
          Tri.face->vertex_angle(Tri.left_vertex), Tri.left_edge->length(), w,
          w->d());
      ...
      ValidPropagation = compute_propagated_parameters(
          Tri.bottom_edge->length() - right_w->pseudo_x(), right_w->pseudo_y(),
          Tri.bottom_edge->length() - right_w->stop(),
          Tri.bottom_edge->length() - right_w->start(),
          Tri.face->vertex_angle(Tri.right_vertex), Tri.right_edge->length(),
          right_w, right_w->d());
...
inline windows_state
GeodesicAlgorithmExact::check_between_two_windows(interval_pointer &w1,
                                                  interval_pointer &w2) {
  ...
  if ((w1->start() >= w2->start()) && (w1->start() <= w2->stop())) // w1->start
  {
    ...
    CalculateIntersectionPoint(w2->pseudo_x(), w2->pseudo_y(), w1->start(), 0,
                               w1->pseudo_x(), w1->pseudo_y(), w1->stop(), 0,
                               Intersect_X, Intersect_Y);

    if ((Intersect_Y <= 0) && (Intersect_Y >= w1->pseudo_y()) &&
        (Intersect_Y >= w2->pseudo_y())) {
      double d1, d2;
      d1 = w1->d() + sqrt((w1->start() - w1->pseudo_x()) *
                              (w1->start() - w1->pseudo_x()) +
                          (w1->pseudo_y()) * (w1->pseudo_y()));
      d2 = w2->d() + sqrt((w1->start() - w2->pseudo_x()) *
                              (w1->start() - w2->pseudo_x()) +
                          (w2->pseudo_y()) * (w2->pseudo_y()));
  ...
  if ((w1->stop() >= w2->start()) && (w1->stop() <= w2->stop())) // w1->stop
  {
    ...
    // judge the order of the two windows
    CalculateIntersectionPoint(w2->pseudo_x(), w2->pseudo_y(), w1->stop(), 0,
                               w1->pseudo_x(), w1->pseudo_y(), w1->start(), 0,
                               Intersect_X, Intersect_Y);
    ...
    if ((Intersect_Y <= 0) && (Intersect_Y >= w1->pseudo_y()) &&
      ...
      double d1, d2;
      d1 = w1->d() +
           sqrt((w1->stop() - w1->pseudo_x()) * (w1->stop() - w1->pseudo_x()) +
                (w1->pseudo_y()) * (w1->pseudo_y()));
      d2 = w2->d() +
           sqrt((w1->stop() - w2->pseudo_x()) * (w1->stop() - w2->pseudo_x()) +
                (w2->pseudo_y()) * (w2->pseudo_y()));
  ...
  if ((w2->start() >= w1->start()) && (w2->start() <= w1->stop())) // w2->start
  {
    ...
    CalculateIntersectionPoint(w1->pseudo_x(), w1->pseudo_y(), w2->start(), 0,
                               w2->pseudo_x(), w2->pseudo_y(), w2->stop(), 0,
                               Intersect_X, Intersect_Y);

    if ((Intersect_Y <= 0) && (Intersect_Y >= w1->pseudo_y()) &&
        (Intersect_Y >= w2->pseudo_y())) {
      double d1, d2;
      d1 = w1->d() + sqrt((w2->start() - w1->pseudo_x()) *
                              (w2->start() - w1->pseudo_x()) +
                          (w1->pseudo_y()) * (w1->pseudo_y()));
      d2 = w2->d() + sqrt((w2->start() - w2->pseudo_x()) *
                              (w2->start() - w2->pseudo_x()) +
                          (w2->pseudo_y()) * (w2->pseudo_y()));
  ...
  if ((w2->stop() >= w1->start()) && (w2->stop() <= w1->stop())) // w2->stop
  {
    ...
    // judge the previous order of the two windows
    CalculateIntersectionPoint(w1->pseudo_x(), w1->pseudo_y(), w2->stop(), 0,
                               w2->pseudo_x(), w2->pseudo_y(), w2->start(), 0,
                               Intersect_X, Intersect_Y);
    ...
    if ((Intersect_Y <= 0) && (Intersect_Y >= w1->pseudo_y()) &&
      ...
      double d1, d2;
      d1 = w1->d() +
           sqrt((w2->stop() - w1->pseudo_x()) * (w2->stop() - w1->pseudo_x()) +
                (w1->pseudo_y()) * (w1->pseudo_y()));
      d2 = w2->d() +
           sqrt((w2->stop() - w2->pseudo_x()) * (w2->stop() - w2->pseudo_x()) +
                (w2->pseudo_y()) * (w2->pseudo_y()));
  ...
  if (w1->start() >= w2->stop()) {
    ...
    // judge the previous order of the two windows
    CalculateIntersectionPoint(w1->pseudo_x(), w1->pseudo_y(), w1->start(), 0,
                               w2->pseudo_x(), w2->pseudo_y(), w2->stop(), 0,
                               Intersect_X, Intersect_Y);
    ...
    if ((PointInTriangle(Intersect_X, Intersect_Y, Tri.bottom_edge->length(),
                         Cx, Cy)) &&
        (Intersect_Y <= 0) && (Intersect_Y >= w1->pseudo_y()) &&
        (Intersect_Y >= w2->pseudo_y())) {
      double d1, d2;
      d1 =
          w1->d() +
          sqrt((Intersect_X - w1->pseudo_x()) * (Intersect_X - w1->pseudo_x()) +
               (Intersect_Y - w1->pseudo_y()) * (Intersect_Y - w1->pseudo_y()));
      d2 =
          w2->d() +
          sqrt((Intersect_X - w2->pseudo_x()) * (Intersect_X - w2->pseudo_x()) +
               (Intersect_Y - w2->pseudo_y()) * (Intersect_Y - w2->pseudo_y()));
  ...
  if (w2->start() >= w1->stop()) {
    ...
    // judge the previous order of the two windows
    CalculateIntersectionPoint(w2->pseudo_x(), w2->pseudo_y(), w2->start(), 0,
                               w1->pseudo_x(), w1->pseudo_y(), w1->stop(), 0,
                               Intersect_X, Intersect_Y);
...
inline void
GeodesicAlgorithmExact::pairwise_windows_checking(list_pointer &list) {
  if (list->empty())
    return;
...
//------------------------- main operation ----------------------------
inline void
GeodesicAlgorithmExact::propagate_one_windows_list(list_pointer &list) {
  if (list->empty())
    return;
  ...
  while (!m_vertex_queue.empty()) {
    ...
    for (unsigned i = 0; i < vert->adjacent_faces().size(); ++i) {
      ...
      vertex_pointer vert_it =
          (edge_it->adjacent_faces().size() < 2)
              ? NULL
              : edge_it->opposite_face(vert->adjacent_faces()[i])
                    ->opposite_vertex(edge_it);
      if (edge_it->adjacent_faces().size() < 2 ||
          vert_it->state() != Vertex::OUTSIDE) {
        if (!interval_list_0(edge_it)->empty())
          m_list_queue.push(interval_list_0(edge_it));
        if (!interval_list_1(edge_it)->empty())
          m_list_queue.push(interval_list_1(edge_it));
    ...
    while (!m_list_queue.empty()) {
      ...
      if (!is_boundary) {
        ...
        // merge windows lists
        if (!wl_left.empty()) {
          // in VTP, both "PrimeMerge" and "SecondMerge" connect window lists in
          // an order-free way
          ...
          if (((Tri.left_edge->v0()->state() == Vertex::INSIDE) ||
               (Tri.left_edge->v1()->state() == Vertex::INSIDE)) &&
              (!Tri.left_list->empty()))
        ...
        if (!wl_right.empty()) {
          ...
          // push updated list into M_LIST_QUEUE
          if (((Tri.right_edge->v0()->state() == Vertex::INSIDE) ||
               (Tri.right_edge->v1()->state() == Vertex::INSIDE)) &&
              (!Tri.right_list->empty()))
    ...
    if (m_vertex_queue.size() > m_queue_max_size)
      m_queue_max_size = (int)m_vertex_queue.size();
  }
...
inline void GeodesicAlgorithmExact::print_statistics() {
  ...
  // std::cout << std::endl;
  std::cout << "Peak number of intervals on wave-front " << m_windows_peak
            << std::endl;
  std::cout << "uses about " << memory * m_windows_peak / 1e6 << "MB of memory"
            << std::endl;
  std::cout << "total interval propagation number " << m_windows_propagation
            << std::endl;

...
template <> struct hash<yocto::vec2i> {
...
vec3f eval_position(const vector<vec3i> &triangles,
  ...
  return interpolate_triangle(positions[x], positions[y], positions[z],
                              sample.uv);
}
...
unfold_triangle init_flat_triangle(const vector<vec3f> &positions,
                                   const vec3i &tr) {
...
unfold_triangle triangle_coordinates(const vector<vec3i> &triangles,
  ...
  auto point_coords =
      interpolate_triangle(first[0], first[1], first[2], point.uv);
...
vector<unfold_triangle> unfold_strip(const vector<vec3i> &triangles,
  ...
  for (auto i = 1; i < strip.size(); i++) {
    ...
    assert(k != -1);
    coords[i] = unfold_face(triangles, positions, adjacencies, coords[i - 1],
                            strip[i - 1], k);
  }
...
float length_by_flattening(const vector<vec3i> &triangles,
  ...
  for (auto i = 1; i < strip.size(); i++) {
    auto k = find_in_vec(adjacencies[strip[i - 1]], strip[i]);
    assert(k != -1);
    auto tr = unfold_face(triangles, positions, adjacencies, coords[i - 1],
                          strip[i - 1], k);
    coords[i] = tr;
  }
...
vector<int> triangle_fan(const vector<vec3i> &adjacencies, int face, int k,
                         bool clockwise) {
...
int opposite_face(const vector<vec3i> &triangles,
  ...
  for (auto i = 0; i < 3; ++i) {
    if (find_in_vec(triangles[triangle[i]], vid) < 0)
      return triangle[i];
  }
  return -1;
}
...
int opposite_vertex(const vec3i &triangle, const vec2i &edge) {
  for (auto i = 0; i < 3; ++i) {
    if (triangle[i] != edge.x && triangle[i] != edge.y)
      return triangle[i];
  }
  return -1;
}
...
void meandering_triangles(const vector<float> &field, float Isoline,
                          int selected_tag, int t0, int t1,
                          vector<vec3i> &triangles, vector<int> &tags,
...
static float opposite_nodes_arc_length(const vector<vec3f> &positions, int a,
                                       int c, const vec2i &edge) {
  ...
  auto len =
      dot(ba, ba) + dot(bc, bc) - length(ba) * length(bc) * 2 * cos_alpha_beta;
...
static void connect_opposite_nodes(geodesic_solver &solver,
                                   const vector<vec3f> &positions,
                                   const vec3i &tr0, const vec3i &tr1,
                                   const vec2i &edge) {
...
geodesic_solver make_geodesic_solver(const vector<vec3i> &triangles,
  ...
  for (auto face = 0; face < triangles.size(); face++) {
    for (auto k = 0; k < 3; k++) {
      ...
      auto neighbor = adjacencies[face][k];
      if (face < neighbor) {
        connect_opposite_nodes(solver, positions, triangles[face],
                               triangles[neighbor], {a, b});
      }
    }
  }
  return solver;
...
make_dual_geodesic_solver(const vector<vec3i> &triangles,
                          const vector<vec3f> &positions,
                          const vector<vec3i> &adjacencies) {
...
vector<vector<float>>
compute_angles(const vector<vec3i> &triangles, const vector<vec3f> &positions,
               const vector<vec3i> &adjacencies, const vector<vector<int>> &v2t,
               vector<float> &total_angles, bool with_opposite) {
  ...
  for (auto i = 0; i < positions.size(); ++i) {
    ...
    for (auto j = 0; j < star.size(); ++j) {
      ...
      if (with_opposite) {
        ...
        auto tr_opp =
            unfold_face(triangles, positions, adjacencies, tr2d, tid, k);
        ...
        auto phi = angle(positions[tr3d[(k + 1) % 3]] - positions[i],
                         positions[tr3d[(k + 2) % 3]] - positions[i]);
        theta += phi;
        curr_angles.push_back(phi);
      }
    }
...
vector<int> sample_vertices_poisson(const geodesic_solver &solver,
                                    int num_samples) {
...
vector<vector<float>> compute_voronoi_fields(const geodesic_solver &solver,
                                             const vector<int> &generators) {
...
vector<vec3f> colors_from_field(const vector<float> &field, float scale,
                                const vec3f &c0, const vec3f &c1) {
...
static int adjacent_face(const vector<vec3i> &triangles,
                         const vector<vec3i> &adjacency, int face,
                         const vec2i &edge) {
  ...
  for (auto k = 0; k < 3; ++k) {
    ...
    auto e = vec2i{x, y};
    if (e == edge)
      return adjacency[face][k];
    if (vec2i{e.y, e.x} == edge)
      return adjacency[face][k];
  }
  return -1;
}
...
step_from_edge_to_edge(const vec3f &point, const vec3f &a, const vec3f &b,
                       const vec3f &c, const vec3f &direction,
                       float epsilon = 0.0001f) {
  //      b
  //     /\
  //    /  \
  ...
  if (right_side) {
    ...
    if (below_edge)
      return {1.0f - epsilon, false};
    auto x = step_from_point_to_edge(front, left, direction);
    return {x, false};
  }
}

static surface_path::vertex
step_from_point(const vector<vec3i> &triangles, const vector<vec3f> &positions,
                const vector<vec3i> &adjacency, const vector<int> &tags,
                const vector<float> &field, int vertex, int start_face,
                int tag = -1, float epsilon = 0.0001f) {
  auto opposite_edge = [](int vertex, const vec3i &tr) -> vec2i {
    for (auto i = 0; i < 3; ++i) {
      if (tr[i] == vertex)
        return {tr[(i + 1) % 3], tr[(i + 2) % 3]};
    }
    return {-1, -1};
  };
...
surface_path integrate_field(const vector<vec3i> &triangles,
                             ...
                             const vector<int> &tags, int tag,
                             const vector<float> &field, int from) {
  auto opposite_vertex = [](const vec3i &tr, const vec2i &edge) -> int {
    for (auto i = 0; i < 3; ++i) {
      if (tr[i] != edge.x && tr[i] != edge.y)
        return tr[i];
  ...
  auto lerp = step_from_point(triangles, positions, adjacency, tags, field,
                              from, -1, tag);
  if (lerp.face == -1)
    return {};
...
surface_path integrate_field(const vector<vec3i> &triangles,
                             ...
                             const vector<vec3i> &adjacency,
                             const vector<int> &tags, int tag,
                             const vector<float> &field, int from, int to) {
  auto opposite_vertex = [](const vec3i &tr, const vec2i &edge) -> int {
    for (int i = 0; i < 3; ++i) {
      if (tr[i] != edge.x && tr[i] != edge.y)
        return tr[i];
  ...
  for (auto i = 0; i < num_steps; i++) {
    ...
    if (old_alpha == 0 || old_alpha == 1) {
      ...
      auto lerp = step_from_point(triangles, positions, adjacency, tags, field,
                                  vertex, old_face);
      lerps.push_back(lerp);
      if (lerp.alpha == 0 && lerp.edge.x == to)
        break;
      if (lerp.alpha == 1 && lerp.edge.y == to)
        break;
      continue;
    }
...
vector<vec3f> make_positions_from_path(const surface_path &path,
                                       const vector<vec3f> &mesh_positions) {
...
surface_path integrate_field(const vector<vec3i> &triangles,
                             ...
                             const vector<int> &tags, int tag,
                             const vector<float> &field, int from) {
  return integral_paths::integrate_field(triangles, positions, adjacency, tags,
                                         tag, field, from);
}
surface_path integrate_field(const vector<vec3i> &triangles,
                             const vector<vec3f> &positions,
...
nodes_around_point(const vector<vec3i> &triangles,
                   const vector<vec3f> &positions,
...
vector<float>
solve_with_parents(const geodesic_solver &solver,
                   const vector<pair<int, float>> &sources_and_dist,
                   const vector<pair<int, float>> &targets,
                   vector<int> &parents, bool with_parents = false) {
...
int set_source_child(const vector<int> &parents, int target_parent,
                     vector<int> &path) {
  auto stop = false;
...
vector<int> point_to_point_geodesic_path(const geodesic_solver &solver,
  ...
  auto source_nodes =
      nodes_around_point(triangles, positions, adjacencies, source);
  auto target_nodes =
      nodes_around_point(triangles, positions, adjacencies, target);
  auto parents = vector<int>{};
  auto distances =
      solve_with_parents(solver, source_nodes, target_nodes, parents);
...
vector<int> compute_pruned_geodesic_paths(const geodesic_solver &solver,
                                          const vector<int> &sources,
                                          int end_vertex) {
  auto parents = vector<int>(solver.graph.size(), -1);
  auto distances = vector<float>(solver.graph.size(), flt_max);
...
vector<float> compute_geodesic_distances(const geodesic_solver &solver,
  ...
  for (auto i = 0; i < sources.size(); ++i) {
    auto curr_nodes =
        nodes_around_point(triangles, positions, adjacencies, sources[i]);
    source_nodes.insert(source_nodes.end(), curr_nodes.begin(),
                        curr_nodes.end());
  }
  if (source_nodes.size() > 1)
    check_nodes(source_nodes);
...
void heuristic_visit_geodesic_graph(vector<float> &field,
                                    const dual_geodesic_solver &solver,
                                    const vector<vec3i> &triangles,
                                    const vector<vec3f> &positions, int start,
                                    int end, Update &&update, Stop &&stop,
                                    Exit &&exit) {
...
vector<int> strip_on_dual_graph(const dual_geodesic_solver &solver,
  ...
  heuristic_visit_geodesic_graph(field, solver, triangles, positions, start,
...
static void fill_strip(vector<int> &strip, const vector<vector<int>> &v2t,
                       int vid, int first, int last, bool nei_is_dual,
                       bool ccw) {
  ...
  if (start == end) {
    if (strip.back() != star[start])
      strip.push_back(star[start]);
  } else if (ccw) {
    if (strip.back() == star[start])
      start = (start + 1) % s;
    if (start > end)
...
static int get_entry(vector<int> &strip, const geodesic_solver &solver,
                     ...
                     const vector<vec3i> &adjacencies,
                     const vector<vector<int>> &v2t,
                     const vector<vector<float>> &angles, int parent,
                     const mesh_point &p) {
  strip = {p.face};
  if (auto [is_vert, offset] = point_is_vert(p); is_vert) {
    auto vid = triangles[p.face][offset];
    ...
    if (h == -1) {
      for (auto i = 0; i < 3; ++i) {
        ...
        if (h != -1) {
          strip.push_back(adj);
          auto entry =
              node_is_neighboor(solver, parent, triangles[adj][(h + 1) % 3]);
          assert(entry >= 0);
          return entry + 1;
        }
      }
      ...
      auto entry =
          node_is_neighboor(solver, parent, triangles[p.face][(h + 1) % 3]);
      assert(entry >= 0);
      return entry + 1;
    }
  }
...
static vector<int>
short_strip(const geodesic_solver &solver, const vector<vec3i> &triangles,
...
vector<int>
get_strip(const geodesic_solver &solver, const vector<vec3i> &triangles,
          const vector<vec3f> &positions, const vector<vec3i> &adjacencies,
          const vector<vector<int>> &v2t, const vector<vector<float>> &angles,
          const mesh_point &source, const mesh_point &target) {
  if (target.face == source.face)
    return {target.face};
  ...
  if (N == 0) {
    ...
    if (strip.back() == strip_to_point.back())
      strip_to_point.pop_back();
...
vector<vec3f> path_positions(const geodesic_path &path,
  ...
  for (auto i = 0; i < path.lerps.size(); i++) {
    auto e = get_edge(triangles, positions, adjacencies, path.strip[i],
                      path.strip[i + 1]);
    if (e == vec2i{-1, -1})
      continue;
    auto x = path.lerps[i];
...
std::tuple<float, vector<float>>
path_length_and_paramters(const vector<vec3f> &positions) {
  auto len = 0.0f;
  auto parameter_t = vector<float>(positions.size());
  for (auto i = 0; i < positions.size(); i++) {
    if (i)
...
geodesic_path straightest_path(const vector<vec3i> &triangles,
                               const vector<vec3f> &positions,
                               const vector<vec3i> &adjacencies,
                               const mesh_point &start, const vec2f &direction,
                               float path_length) {
  ...
  while (len < path_length) {
    ...
    for (auto k = 0; k < 3; ++k) {
      if (adjacencies[face][k] == prev_face)
        continue;
      auto left = coords[k];
      ...
      if (t0 > 0 && t1 >= 0 && t1 <= 1) {
        ...
        if (t0 < path_length) {
          ...
          coords =
              unfold_face(triangles, positions, adjacencies, coords, face, k);
...
static vector<pair<vec2f, vec2f>>
get_portals(const vector<vec3i> &adjacencies,
  ...
  auto end = interpolate_triangle(coords.back()[0], coords.back()[1],
                                  coords.back()[2], to.uv);
...
bool path_check_strip(const vector<vec3i> &adjacencies,
                      const vector<int> &strip) {
...
static vector<float> funnel(const vector<pair<vec2f, vec2f>> &portals,
                            int &max_index) {
  // Find straight path.
...
static vector<int> fix_strip(const vector<vec3i> &adjacencies,
                             const vector<int> &strip, int index, int k,
                             bool left) {
...
static void optimize_path(geodesic_path &path, const vector<vec3i> &triangles,
  ...
  auto init_coords =
      unfold_strip(triangles, positions, adjacencies, path.strip, path.start);
  auto init_portals =
      get_portals(adjacencies, init_coords, path.strip, path.end);
...
inline int find_adjacent_triangle(const vec3i &triangle,
                                  const vec3i &adjacent) {
  for (int i = 0; i < 3; i++) {
...
inline int find_adjacent_triangle(const vector<vec3i> &triangles, int face,
                                  int neighbor) {
...
unfold_triangle unfold_face(const vector<vec3i> &triangles,
                            const vector<vec3f> &positions,
                            const unfold_triangle &tr, int face, int neighbor) {
...
vector<unfold_triangle> unfold_strip(const vector<vec3i> &triangles,
  ...
  for (auto i = 1; i < strip.size(); i++) {
    coords[i] = unfold_face(triangles, positions, coords[i - 1], strip[i - 1],
                            strip[i]);
  }
...
// Create sequence of 2D segments (portals) needed for funneling.
static vector<pair<vec2f, vec2f>>
make_funnel_portals(const vector<vec3i> &triangles,
                    const vector<unfold_triangle> &coords,
...
inline vector<pair<vec2f, vec2f>>
unfold_funnel_portals(const vector<vec3i> &triangles,
...
static void straighten_path(geodesic_path &path, const vector<vec3i> &triangles,
                            ...
                            const vector<vec3i> &adjacencies) {
  auto index = -1, vertex = -1;
  auto init_portals = unfold_funnel_portals(triangles, positions, path.strip,
                                            path.start, path.end);
  path.lerps = funnel(init_portals, index);
  ...
  for (auto i = 0; i < path.strip.size() * 2 && index != -1; i++) {
    ...
    auto portals = unfold_funnel_portals(triangles, positions, path.strip,
                                         path.start, path.end);
    path.lerps = funnel(portals, index);
  }
}
...
mesh_point eval_path_point(const geodesic_path &path,
  ...
  auto parameter_t =
      path_parameters(path_positions(path, triangles, positions, adjacencies));
...
bool save_mesh(const string &filename, const vector<vec3i> &triangles,
  ...
  if (ext == ".ply" || ext == ".PLY") {
    ...
    if (!save_ply(filename, ply, error))
      return false;
    return true;
  } else if (ext == ".obj" || ext == ".OBJ") {
    auto obj_guard = std::make_unique<obj_scene>();
    ...
    auto err = ""s;
    if (!save_obj(filename, obj, error))
      return false;
    return true;
  } else {
    return format_error();
...
bool save_mesh(const string &filename, const vector<vec3i> &triangles,
               const vector<vec3f> &positions, const vector<vec3f> &normals,
               const vector<vec2f> &texcoords, const vector<vec3b> &colors,
               string &error, bool ascii, bool flip_texcoord) {
  auto format_error = [filename, &error]() {
    error = filename + ": unknown format";
    return false;
...
vector<string> mesh_stats(const vector<vec3i> &triangles,
                          ...
                          const vector<vec3f> &colors, bool verbose) {
